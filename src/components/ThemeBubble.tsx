...
const clampThemePos = (x: number, y: number) => ({
  x: Math.min(Math.max(x, dockClearance()), Math.max(dockClearance(), window.innerWidth - Math.min(window.innerWidth * 0.94, THEME_PANEL_W) - VIEWPORT_MARGIN)),
...
export const ThemeBubble = () => {
  ...
  const handleDragMove = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    const resolved = resolveNonOverlapping(
      PANEL_ID,
      Math.min(Math.max(nextX, dockClearance()), maxX),
